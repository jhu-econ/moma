...
Utility depends on both consumption $c_t$ and leisure $z_t$:

$$u(c_t, z_t)$$

**Constraints:**

- Time allocation: $\ell_t + z_t = 1$ (labor + leisure = 1)
- Labor income: $y_t = W_t \ell_t = W_t(1 - z_t)$
...
**Log utility** ($\rho = 1$) eliminates the wage effect on consumption growth:

$$c_2/c_1 = R\beta$$

Labor supply profile:

$$\frac{1-\ell_2}{1-\ell_1} = \frac{R\beta \, W_1}{W_2}$$

Work harder when wages are higher: the intertemporal substitution of labor.
...
With **log utility** ($\rho = 1$):

$$c_{1,t} = \frac{W_{1,t}}{1+\beta}$$

$$a_{1,t} = W_{1,t} \cdot \frac{\beta}{1+\beta}$$

The saving rate $\frac{\beta}{1+\beta}$ is constant, a strong prediction of log utility.
...
This is a **nonlinear first-order difference equation** in per-capita capital.

The derivative:

$$\frac{dk_{t+1}}{dk_t} = \varepsilon \mathcal{Q} \, k_t^{\varepsilon - 1}$$

Since $\varepsilon < 1$, the mapping is concave and converges monotonically.
...
N_vals = np.linspace(1.0, 1.1, 200)
beta_fix = 0.96
...
where the Hebrew letter $\beth$ (beth) is the social planner's discount factor. The resource constraint is:

$$\underbrace{K_t + F(K_t, \mathcal{N}_t)}_{\text{Sources}} = \underbrace{K_{t+1} + \mathcal{N}_t c_{1,t} + \mathcal{N}_{t-1} c_{2,t}}_{\text{Uses}}$$

The socially optimal steady state satisfies:

$$1 + f'(\bar{k}^*) = N \beth^{-1}$$

## Dynamic Efficiency and the Golden Rule

Per-capita steady-state consumption:

$$\bar{c} = f(\bar{k}) - n\bar{k}$$

The **Golden Rule** maximizes $\bar{c}$: $f'(\bar{k}^{**}) = n$, yielding:
...
- $\bar{k}^{**}$: golden rule (maximizes steady-state consumption)

There is "no particular relationship" between $\bar{k}$ and $\bar{k}^*$.
